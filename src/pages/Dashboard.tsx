...
import { PRIORITY_COLORS_DISPLAY } from '../utils/colors';
import {
  BarChart,
  Bar,
  XAxis,
...
export function Dashboard() {
  ...
  const handleCreateBoard = () => {
    if (currentWorkspaceId && !isCreatingBoard) {
  ...
  }, [workspace, statusConfigs]);

  const { allTasks, statusDistribution, priorityDistribution, tasksThisWeek, completionRate, avgSubtasks } = stats;
                ...
                <BarChart data={priorityDistribution} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                  ...
                  >
                    {priorityDistribution.map((entry) => (
                      <Cell
                        key={`cell-${entry.name}`}
                        fill={PRIORITY_COLORS_DISPLAY[entry.name as keyof typeof PRIORITY_COLORS_DISPLAY] || '#6B7280'}
                      />
                    ))}
                  </Bar>
                </BarChart>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          >
            ...
            <div className="text-left">
              <h3 className="font-medium">{isCreatingBoard ? 'Creating...' : 'New Board'}</h3>
